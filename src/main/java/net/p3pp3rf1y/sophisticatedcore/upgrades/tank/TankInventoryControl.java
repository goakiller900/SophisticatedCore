...
public class TankInventoryControl extends UpgradeInventoryControlBase {
	...
	public boolean handleMouseReleased(MouseButtonEvent event) {
		if (event.x() < screen.sophisticatedCore_getGuiLeft() + getTankLeft() || event.x() >= screen.sophisticatedCore_getGuiLeft() + getTankLeft() + 18 || event.y() < screen.sophisticatedCore_getGuiTop() + pos.y()
				|| event.y() >= screen.sophisticatedCore_getGuiTop() + pos.y() + height) {
			return false;
		}
		...
		ClientPlayNetworking.send(new TankClickPayload(upgradeSlot));
	...
	private void extractFluid(GuiGraphicsExtractor guiGraphics) {
		...
		int color = fluidModel.tintSource() != null ? fluidModel.tintSource().color(fluidState.createLegacyBlock()) : -1;
