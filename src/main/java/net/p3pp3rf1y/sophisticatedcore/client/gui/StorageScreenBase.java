...
public abstract class StorageScreenBase<S extends StorageContainerMenuBase<?>> extends AbstractContainerScreen<S>
			...
			if (menu.canDisableUpgrade(slot)) {
				...
				ToggleButton<Boolean> upgradeSwitch = new ToggleButton<>(new Position(leftPos - 22, switchTop), ButtonDefinitions.UPGRADE_SWITCH,
					...
					protected void extractWidget(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
						if (getMenu().isUpgradeRunnable(finalSlot)) {
	...
	public Slot getHoveredSlot(double mouseX, double mouseY) {
		...
		if (inventoryScrollPanel != null) {
			...
			if (result.isPresent()) {
				...
				return getMenu().isStorageInventorySlot(slot.index) && getMenu().isInaccessibleSlot(slot.index) ? null : slot;
			}
			for (Slot slot : menu.slots) {
				if (!getMenu().isStorageInventorySlot(slot.index) && isHovering(slot, mouseX, mouseY) && slot.isActive()) {
					return slot;
				}
			}
			return null;
			...
			for (Slot slot : menu.slots) {
				if (isHovering(slot, mouseX, mouseY) && slot.isActive()) {
					if (getMenu().isStorageInventorySlot(slot.index) && getMenu().isInaccessibleSlot(slot.index)) {
						return null;
					}
					return slot;
				}
	...
	private void tryQuickMoveSlot(int button, Slot slot, Slot slot2) {
		...
		if (slot2.mayPickup(minecraft.player) && slot2.hasItem() && slot2.container == slot.container) {
			...
			if (ItemStack.isSameItemSameComponents(lastQuickMoved, slotItem)) {
				if (slotItem.getCount() > slotItem.getMaxStackSize()) {
					ClientPlayNetworking.send(new TransferFullSlotPayload(slot2.index));
				} else {
	...
	public boolean mouseClicked(MouseButtonEvent event, boolean doubleClicked) {
		...
		if (event.hasShiftDown() && event.hasControlDown() && slot instanceof StorageInventorySlot && event.button() == 0) {
			ClientPlayNetworking.send(new TransferFullSlotPayload(slot.index));
			return true;
		}
