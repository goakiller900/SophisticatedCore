...
public class GuiHelper {
	...
	public static void extractTooltip(Screen screen, GuiGraphicsExtractor guiGraphics, ItemStack tooltipStack, List<Component> components,
		...
		List<Either<FormattedText, TooltipComponent>> elements = components.stream()
				.<Either<FormattedText, TooltipComponent>>map(Either::left)
				.collect(ArrayList::new, ArrayList::add, ArrayList::addAll);
		tooltipComponent.ifPresent(component -> elements.add(1, Either.right(component)));
		List<ClientTooltipComponent> list = gatherTooltipComponents(elements, x, screen.width, screen.getFont());
		guiGraphics.tooltip(screen.getFont(), list, x, y, DefaultTooltipPositioner.INSTANCE, null);
	}

	// Splitting is always called so that new lines in translations are properly wrapped.
	public static List<ClientTooltipComponent> gatherTooltipComponents(List<? extends FormattedText> textElements, int mouseX, int screenWidth,
		...
		List<Either<FormattedText, TooltipComponent>> elements = textElements.stream()
				.<Either<FormattedText, TooltipComponent>>map(Either::left)
				.toList();
		return gatherTooltipComponents(elements, mouseX, screenWidth, fallbackFont);
	}

	private static List<ClientTooltipComponent> gatherTooltipComponents(List<Either<FormattedText, TooltipComponent>> elements, int mouseX,
			int screenWidth, Font font) {
		// text wrapping
